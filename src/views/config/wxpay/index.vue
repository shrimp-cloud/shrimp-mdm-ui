<template>
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" label-width="68px" @submit.native.prevent>
        ...
        <el-form-item prop="appId">
          <el-input v-model="queryParams.mchId" placeholder="商户ID" clearable style="width: 160px" @keyup.enter="handleQuery"/>
        </el-form-item>
         <el-form-item>
      ...
      <el-table v-loading="loading" :height="tableHeight" :data="dataList">
        ...
        <el-table-column label="应用ID" align="left" prop="appId" min-width="120" :show-overflow-tooltip="true"/>
        <el-table-column label="商户ID" align="left" prop="mchId" min-width="120" :show-overflow-tooltip="true"/>
        <el-table-column label="沙箱" align="center" prop="useSandboxEnv" width="80">
          <template #default="scope"><dict-tag :options="BOOLEAN" :value="scope.row.useSandboxEnv"/></template>
        </el-table-column>
...
<script setup name="WxpayConfig">
import {wxpayConfigPage, wxpayConfigRemove} from "@/api/wxpay";
...
function getList() {
  loading.value = true;
  wxpayConfigPage(queryParams.value).then(res => {
    const data = res.data;
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除:"' + row.appId + '"？').then(() => {
    wxpayConfigRemove({id: row.id}).then(res => {
      if (res.code === 1) {
        getList();
